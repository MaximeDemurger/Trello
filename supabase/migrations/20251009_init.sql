...
create trigger set_public_profiles_updated_at before update on public.profiles for each row execute function public.set_updated_at();
...
-- Now that organization_members exists, add org policies that depend on it
...
drop policy if exists "Members can manage invites" on public.organization_invites;
...
drop policy if exists "Invited user can view their invite" on public.organization_invites;
...
drop policy if exists "Org members can read boards" on public.boards;
...
drop policy if exists "Creators can update boards" on public.boards;
create policy "Creators can update boards" on public.boards for update using (created_by = auth.uid());
drop policy if exists "Creators can delete boards" on public.boards;
create policy "Creators can delete boards" on public.boards for delete using (created_by = auth.uid());
drop trigger if exists set_public_boards_updated_at on public.boards;
create trigger set_public_boards_updated_at before update on public.boards for each row execute function public.set_updated_at();
...
create trigger set_public_items_updated_at before update on public.items for each row execute function public.set_updated_at();
